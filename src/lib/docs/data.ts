/**
 * Curated API data for MCP output, optimized for LLM consumption.
 * Adapted from functype/src/cli/data.ts
 */
...
const getFunctypeVersion = (): string => {
  try {
    ...
    for (let i = 0; i < 5; i++) {
      try {
        ...
        if (pkg.name === "functype" && pkg.version) return pkg.version
      } catch {
        // continue searching
      }
      dir = dirname(dir)
...
export const VERSION = getFunctypeVersion()
